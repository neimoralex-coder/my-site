...
useEffect(() => {
  ...
  const handleMediaUpload = async (
    ...
    try {
      ...
      // В конфиг сайта сохраняем не сам файл, а публичную ссылку из Supabase Storage.
      // Поэтому файл будет отображаться на всех устройствах, а не только локально.
      onChange(uploaded.url);

      event.target.value = '';

      alert(
        uploaded.wasCompressed
          ? `Фото сжато с ${uploaded.originalSizeMb} МБ до ${uploaded.finalSizeMb} МБ и загружено.`
          : `Файл загружен. Размер: ${uploaded.finalSizeMb} МБ.`
      );
    } catch (error) {
      alert(
        error instanceof Error
          ? error.message
          : 'Не удалось загрузить файл. Попробуйте файл поменьше или другой формат.'
      );
    }
  };
  ...
  const mediaField = (value: string, onChange: (value: string) => void, placeholder: string) => (
    <div className="space-y-2">
      ...
      <div className="rounded-xl border border-blue-100 bg-blue-50 px-3 py-2 text-xs leading-relaxed text-blue-700">
        Фото: JPG, PNG, WebP, GIF, AVIF — автоматически сжимается до 2 МБ и 1920 px.
        Видео: MP4, WebM, MOV, M4V — до 20 МБ. Если файл не подходит, загрузка будет отклонена.
      </div>
